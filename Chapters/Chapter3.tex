...
\section{Arquitectura general del sistema} \label{diagrama-bloques-sistema}
El trabajo desarrollado consiste en dos grandes bloques. Por un lado, se encuentra el firmware, que se opera en la plataforma de hardware; por otro, el software de escritorio, que corre en una computadora y se comunica con el firmware. En la figura \ref{fig:sys-arch} se ilustra un diagrama en bloques de la solución implementada.
...
El firmware se encarga de realizar íntegramente el procesamiento del flujo de datos (mensajes MIDI), mientras que el software actúa como interfaz de configuración del comportamiento del equipo. La comunicación entre ambas se realiza mediante un puerto serie virtual, emulado por la interfaz USB del microcontrolador. A través de una serie de comandos, se modifican las propiedades de una estructura que modela el comportamiento de la aplicación (\texttt{Settings}). En la figura \ref{fig:app-settings} se muestra un diagrama de clases que incluye todas las estructuras que componen este modelo.

\begin{figure}[ht]
	\centering
	\includegraphics[width=\textwidth]{./Figures/app-settings.png}
	\caption{Diagrama de clases de las estructuras de datos principales de la aplicación.}
...
\begin{itemize}
	...
	\item \texttt{Preset}: contiene la configuración de todas las entradas y salidas, así como la configuración de ruteo entre ellas. Además, incluye un nombre para que el usuario también pueda identificarla.
	\item \texttt{Settings}: contiene el estado de los dieciséis presets, el número de preset activo, la versión y un identificador único que se utiliza para operaciones de serialización.
\end{itemize}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.7\textwidth]{./Figures/fw-layers.png}
...
\begin{itemize}
	...
	\item \texttt{svc}: capa de servicios de la aplicación. Son módulos de mayor nivel que engloban tareas específicas, relacionadas con los requerimientos de la aplicación.
...
Con base en el esquema de capas propuesto, en la figura \ref{fig:fw-arch} se observa un diagrama de alto nivel de los componentes que integran a la aplicación. En secciones posteriores se realiza un análisis más exhaustivo.
...
\begin{enumerate}
	...
	\item \texttt{svc::led}: controla los LEDs del panel frontal, con la función principal de indicar el preset activo.
	...
	\item \texttt{svc::io}: controla la recepción y transmisión de mensajes MIDI entre las entradas y las salidas. Esto incluye el ruteo y filtrado de mensajes.
\end{enumerate}
...
El servicio \texttt{svc::dial} se encarga de monitorear un encoder rotativo con pulsador y de determinar tanto las pulsaciones del botón como el sentido de giro a partir de dos señales en cuadratura.

En la figura \ref{fig:svc-dial} se ilustra un diagrama en bloques del servicio. Este consiste en una tarea que muestrea periódicamente tres GPIOs configurados como entradas, donde se aplican mecanismos de antirrebote y se procesa la señal en cuadratura para determinar el estado del dial.
...
\begin{itemize}
	\item \texttt{ButtonPressed}: indica una pulsación simple.
	\item \texttt{ButtonDoublePressed}: corresponde a una pulsación doble dentro de una ventana temporal configurable.
	\item \texttt{ButtonLongPressed}: representa una pulsación prolongada, de duración superior al umbral configurado.
	\item \texttt{DialCW}: se emite cuando el encoder gira en sentido horario.
	\item \texttt{DialCCW}: se emite cuando el encoder gira en sentido antihorario.
\end{itemize}
...
El servicio consiste en una tarea que actualiza periódicamente el estado de los LEDs. El uso de una tarea permite implementar animaciones, definidas a través de una FSM (\textit{Finite State Machine}) como se aprecia en la figura \ref{fig:svc-led-fsm}.
...
La función de \texttt{svc::storage} es la de ofrecer una interfaz para leer y escribir la configuración del sistema, modelada mediante la estructura \texttt{app::Settings} mencionada en la sección \ref{diagrama-bloques-sistema}.

\subsection{Diseño del servicio de I/O}
\label{subsection:io}
El servicio \texttt{svc::io} constituye el núcleo de la aplicación, ya que se encarga de procesar el flujo de mensajes MIDI desde las entradas hacia las salidas. En la figura \ref{fig:svc-io} se observa un diagrama en bloques del servicio.
\begin{figure}[ht]
	\centering
...
El proceso de transmisión es un procedimiento más simple, tal como puede observarse en la figura \ref{fig:svc-out-pipe}. La tarea permanece bloqueada hasta que ingresan mensajes en la cola. Ante la llegada de mensajes, se consulta la configuración del filtro de la salida. Si el mensaje no debe ser bloqueado, se transmite por el puerto serie. Para ello es necesario serializar primero el mensaje, responsabilidad que corresponde a la clase \texttt{app::Serializer}.
\begin{figure}[ht]
	\centering
...
El servicio \texttt{svc::comms} gestiona la comunicación con la aplicación de escritorio a través de un puerto serie virtual sobre USB. En la figura \ref{fig:svc-comms} se muestra un diagrama en bloques del servicio.
...
En términos generales, el servicio consta de dos tareas: una destinada a la recepción y decodificación de comandos, y otra destinada a la codificación y transmisión de respuestas. La definición de comandos, respuestas y funciones auxiliares se implementó en una biblioteca independiente llamada \texttt{protocol}, compartida también con la aplicación de escritorio. En la figura \ref{fig:proto-command} se presenta un diagrama de clases de las estructuras principales del protocolo.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./Figures/proto-command.png}
	\caption{Estructuras utilizadas en el protocolo de comandos.}
	\label{fig:proto-command}
\end{figure}

Cada mensaje se representa mediante la plantilla \texttt{proto::Packet<T>}, compuesta por un encabezado (\texttt{proto::Header}), una carga útil y un código CRC (\textit{Cyclic Redundancy Check}) para verificación de integridad. El encabezado contiene el identificador del comando (\texttt{CommandID}) y el tamaño de la carga útil. Además, se incorporó un campo fijo de 32 bits llamado \texttt{magic}, que permite detectar el inicio de una trama, y un campo de versión para preservar compatibilidad futura entre revisiones del protocolo.

Una vez validada y decodificada la trama, el servicio envía eventos a la aplicación principal para su procesamiento y para la generación de la respuesta correspondiente. En la figura \ref{fig:proto-sequence} se ilustra el intercambio de mensajes entre la aplicación y el firmware. Si se detecta un error durante la decodificación o en la verificación de integridad, el servicio responde con un pedido de retransmisión (\texttt{Retx}).

\newpage

De este modo, queda definida la estructura general del protocolo de comunicación. En el apéndice \ref{anexo:comandos} se puede consultar la lista de mensajes desarrollados.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{./Figures/proto-sequence.png}
...
En esta sección se describe el desarrollo de la aplicación de escritorio implementada con Qt 6.
...
Para integrar las funcionalidades provistas por Qt, las clases desarrolladas para la aplicación deben heredar de primitivas expuestas por la API (\textit{Application Programming Interface}). Entre dichas primitivas, la más fundamental es \texttt{QObject} \cite{qt:qobject}. Esta clase da acceso a mecanismos de comunicación entre clases mediante señales, incorporación de metadatos y organización del ciclo de vida mediante árboles de jerarquía, entre otras características \cite{qt:object-model}.
...
Para la implementación de la interfaz gráfica se utilizó QML, un lenguaje declarativo con una sintaxis similar a JavaScript. Qt ofrece el módulo \texttt{QtQuick Controls}, que permite emplear componentes estándar como botones, campos de texto y sliders, entre otros \cite{qt:quickcontrols}. Estos componentes proveen un estilo visual definido por el sistema operativo. No obstante, la documentación también describe mecanismos de personalización que permiten adaptar su apariencia a los requerimientos de UX/UI (\textit{User Experience/User Interface}). En la figura \ref{fig:ui} se muestra el aspecto final de la interfaz gráfica desarrollada.
...
Desde un punto de vista eléctrico, MIDI transmite la información serie sobre un lazo de corriente de 5 mA formado entre el transmisor y el receptor. En la figura \ref{fig:midi-standard-hw} se observa una versión simplificada del esquemático original propuesto por la MMA en \cite{mma:midi_spec}.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{./Figures/midi-standard-hw-2.png}
	\caption{Esquemático de los circuitos de recepción y transmisión MIDI\protect\footnotemark.}
	\label{fig:midi-standard-hw}
\end{figure}

\footnotetext{Imagen adaptada de: MIDI Manufacturers Association, \textit{MIDI 1.0 Detailed Specification}, documento 96-1-4, versión 4.2.1, 1996.}

Un 0 lógico en la UART transmisora cierra el lazo compuesto entre la alimentación; las resistencias $R_a$, $R_b$ y $R_c$ y la etapa adaptadora de impedancia (buffer). Por el contrario, un 1 lógico abre el lazo, por lo que no circula corriente entre el transmisor y el receptor. Dado que el estado de reposo de la UART es con su salida en alto, en un lazo MIDI no circulará corriente hasta que transmitan datos.
...
En 2014, la MMA actualizó la especificación mediante el documento CA-033 \cite{mma:ca033}, donde se modifica el circuito de la figura \ref{fig:midi-standard-hw} para que pueda operar con 3,3 V, y aumentar su robustez frente a problemas de compatibilidad electromagnética y emisión de interferencias. Este documento fue utilizado para el diseño de las placas de entradas y salidas que se describen en las subsecciones \ref{hw-entradas} y \ref{hw-salidas}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{./Figures/circuit-input-stage.png}
...
La placa central consiste en cinco bloques, que se ilustran en la figura \ref{fig:hw-blocks}.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./Figures/hw-blocks.png}
	\caption{Diagrama en bloques de la placa central.}
	\label{fig:hw-blocks}
\end{figure}

Para el diseño de la conexión USB y la etapa reguladora se tomó como referencia el esquemático del kit de desarrollo \cite{st:mb1137-schematic}. En la figura \ref{fig:circuit-usb} se muestra el conector USB con el USBLC6-2 como componente de protección contra descargas electrostáticas \cite{st:usblc6-2}. Por otro lado, se aseguró que la señal diferencial posea una impedancia característica de 90 $\Omega$, de acuerdo a lo especificado por el estándar USB 2.0 \cite{usb:2.0-spec}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{./Figures/circuit-usb.png}
	\caption{Circuito del conector USB.}
	\label{fig:circuit-usb}
\end{figure}

En la figura \ref{fig:circuit-power} se observa la fuente de alimentación. Se modificó el circuito original del kit de desarrollo, ya que se deseaba usar un regulador con menor tensión de dropout. Se optó por utilizar el regulador AP7361C, que ofrece 360 mV nominales de caída \cite{diodes:ap7361c}. Por otro lado, se utilizó el ST890 \cite{st:st890}, un interruptor de alimentación con limitación de corriente, como etapa de protección contra sobrecorriente. Esta corriente se limitó a 500 mA. En el apéndice \ref{anexo:criterios} se puede consultar cómo se dimensionó la resistencia de sensado.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./Figures/circuit-power.png}
	\caption{Circuito de la fuente de alimentación.}
	\label{fig:circuit-power}
\end{figure}

La conexión de las entradas MIDI y de la placa de usuario es directa con el microcontrolador, por lo que no se necesitan componentes adicionales. Por el contrario, la conexión con la placa de salidas posee un buffer cuya función es subir la tensión de las señales (de 3,3 V a 5 V) y adaptar impedancias. En la figura \ref{fig:circuit-out-connector} se muestra el circuito de esta etapa, basado en el óctuple buffer  74HCS244 \cite{ti:sn74hcs244} como componente central.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./Figures/circuit-out-connector.png}
	\caption{Circuito de adaptación hacia la placa de salidas MIDI.}
	\label{fig:circuit-out-connector}
\end{figure}

Además de lo mencionado explícitamente, durante la etapa de diseño se agregaron otros elementos:
\begin{itemize}
	\item Cristal de 10 MHz.
	\item Capacitores de desacople.
	\item Pulsador de reset.
	\item La interfaz de programación del microcontrolador.
	\item Puntos de prueba para todas las señales.
	\item Pads para exponer una UART adicional, usada con fines de depuración.
	\item Pads conectados a pines no utilizados del microcontrolador, para facilitar futuras ampliaciones del hardware.
\end{itemize}

En la figura \ref{fig:mrc-main-board} se muestran las vistas superior e inferior de la placa central.

\begin{figure}[H]
	\centering
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{./Figures/mrc-main-board-top.png}
		\caption{Vista superior.}
		\label{fig:mrc-main-board-top}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth]{./Figures/mrc-main-board-bottom.png}
		\caption{Vista inferior.}
		\label{fig:mrc-main-board-bottom}
	\end{subfigure}

	\caption{Placa central.}
	\label{fig:mrc-main-board}
\end{figure}


\subsection{Diseño del gabinete}

Las placas de circuito impreso fueron diseñadas con la intención de ser alojadas en un gabinete tipo rack de 19 pulgadas, ya que están estandarizados para equipamiento electrónico e informático. Estos gabinetes se dimensionan mediante unidades de rack o RU (1 RU equivale a 1,75 pulgadas). Se estableció como requerimiento usar un gabinete de 1 RU.

Dado que son gabinetes estandarizados, se contactó a un proveedor para coordinar el mecanizado del frente de un gabinete de 100 mm de profundidad. Para ello se modeló el gabinete en Fusion 360, y se colocaron los modelos 3D de las placas exportados en KiCad. Una vez modelado el panel frontal, se exportó el archivo \texttt{.dxf} requerido por el fabricante para realizar el mecanizado. En la figura \ref{fig:gabinete} se observa una fotografía del equipo ensamblado en el gabinete, con la tapa superior removida para mayor visibilidad.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{./Figures/gabinete.jpg}
	\caption{Equipo ensamblado (tapa superior removida).}
	\label{fig:gabinete}
\end{figure}

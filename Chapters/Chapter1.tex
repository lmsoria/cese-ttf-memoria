...
Los equipos se comunican mediante el estándar MIDI (\textit{Musical Instrument Digital Interface}) \cite{mma:midi_spec}, que establece un mecanismo para la transferencia de información entre instrumentos, computadoras y otros dispositivos vinculados a la producción musical.
...
El esquema presenta un cableado más claro y directo, lo que reduce la posibilidad de errores. Además, tanto el controlador como el secuenciador pueden controlar cualquiera de los cuatro sintetizadores, lo que aporta mayor flexibilidad de uso.
%----------------------------------------------------------------------------------------
...
Desarrollado por la firma canadiense MIDI Solutions, este modelo posee cuatro entradas que se combinan y son redirigidas a dos salidas \cite{midisolutions:quadra-merge}. Los equipos que fusionan varias entradas en una o más salidas son conocidos como merge boxes. En la figura \ref{fig:midi-solutions} se observa una fotografía del producto.
...
Desde una perspectiva funcional, puede observarse que dispositivos como el THRU-12 y el Quadra Merge ofrecen soluciones acotadas a la distribución o combinación de señales, sin capacidades de filtrado ni opciones de configuración avanzadas. En cambio, productos como el MRCC y el mioXL brindan ruteo y filtrado configurables, adecuándose a escenarios complejos a costa de una mayor complejidad de uso. En consecuencia, permanece un espacio intermedio para dispositivos que combinen varias entradas y salidas físicas, funciones básicas de ruteo y filtrado y una interfaz de usuario más directa.
...
En consecuencia, se planteó el desafío de desarrollar una versión local de menor costo capaz de:
\begin{itemize}
	\item Soportar múltiples entradas y salidas DIN.
	\item Soportar filtrado de mensajes.
	\item Permitir la configuración del direccionamiento entre entradas y salidas.
\end{itemize}

Finalmente, cabe destacar que pese a que la motivación de este trabajo fue personal, se realizaron encuestas en foros de entusiastas argentinos, con la intención de orientar el desarrollo de forma tal que el trabajo tenga potencial de venta en el futuro. Esto permitiría responder a una necesidad actualmente no cubierta.
...
El objetivo general de este trabajo fue desarrollar el prototipo de un router MIDI de cuatro entradas y ocho salidas, capaz de gestionar el flujo de mensajes mediante funciones de ruteo y filtrado configurables.
